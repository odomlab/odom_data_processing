...
def compute_fastqc_and_insert(lane):
...
if __name__ == '__main__':
  ...
  lanes = Lane.objects.filter(library__code=ARGS.library)

  if ARGS.facility is not None:
    lanes = lanes.filter(facility__code=ARGS.facility)
    
  if ARGS.lanenum is not None:
    lanes = lanes.filter(lanenum=ARGS.lanenum)
  
  for lane in lanes:
    if ARGS.force or lane.laneqc_set.count() == 0:
      compute_fastqc_and_insert(lane)
