    ...
    if barcode:  # If we have a barcode number we need to know the scheme
      ...
      if 'protocol' in rowdict:
        ...
        if re.search(r'\b(?:truseq ?lt)\b', prottag):
          ...
          m = re.match(r'^[A-Z]([0-9]+)', rowdict[code_column], re.I)
          if m:
            # if the N, D, S or whatever is included in barcode, convert it to 'N'.
            adapter = 'NXT_N' + m.group(1)
          else:
            adapter = 'NXT_N' + barcode  # old default was to assume all adapters are N501-N729.
