...
class FlowCellProcess(object):
  ...
  def run(self, run, flowlane=None, fcq=None, destdir=None):
    ...
    if fcq is None:
      fcq = FlowCellQuery(run, flowlane, lims=self.lims,
    ...
    for (lanenum, libset) in fcq.lane_library.items():
      ...
      for lib in libset:
        if fcq.lib_status[lib] in ('new') or not self.db_library_check:
          ...
          runlanes.add((fcq.lims_fc.run_number, lanenum))

    if len(runlanes) == 0:
      LOGGER.info("No ready lanes for run '%s'", run)
    ...
    # for each lane...
    path = destdir
    for (run_id, flowlane) in runlanes:

      # Mark our lane(s) as active (note that each library has its own
      # version of this lane).
      for lane in Lane.objects.filter(runnumber=run_id, flowlane=flowlane):
...
class FlowCellQuery(object):
  ...
  def __init__(self, run_id, flow_lane=None, verbose=False, lims=None,
    ...
    if not lims.running():
      ...
      sys.exit("LIMS not running.")
    self.lims = lims
    self.lims_fc    = self._run_query(run_id, flow_lane)
  ...
  def _run_query(self, run_id, flowlane_num):
    ...
    if lims_fc == None:
      sys.exit("Problem retrieving run with ID %s from LIMS."
               % (run_id,))
    if self.verbose:
      lims_fc.dump()

    if lims_fc.run_number != run_id:
      LOGGER.error(
        "Run id in LIMS result %s != %s from command line.  Quitting.",
        lims_fc.run_number, run_id)
    ...
    if not self.quiet:
      try:
        ...
        print "%s UNK %s" % (run_id, lims_fc.finish_date)
