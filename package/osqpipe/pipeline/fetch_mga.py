...
def fetch_mga (run, flowlane, destination, nameprefix, lims_fc=None):
  ...
  if lims_fc is None:
    ...
    # search lims for a lane on run
    lims_fc = lims.load_run(run)
  ...
  if len(files) == 0:
    LOGGER.info("No files to retrieve for %s_%d", run, lane.lane)
  for lfile in files:
